### Patient Selection ###
# Load the dataset
data <- read.delim("TLCR.txt", na.strings = "NA", check.names = TRUE)
...
data$Histology_Group <- with(data,
...
# 6. Exclude patients with unknown metastatic site data (bone, brain, liver, lung)
data <- subset(data,
...
# Convert Survival.months to numeric safely
...
# 11. Recode Laterality into grouped categories (only Left, Right, Bilateral included)
data$Laterality_grouped <- dplyr::case_when(
  ...
  TRUE ~ NA_character_  # Exclude all others (Unspecified, Not paired, etc.)
)

# 12. Exclude patients with missing/unknown covariates and TX/NX stages
data <- subset(data,
...
data <- subset(data,
               !( 
                 (!is.na(Derived.AJCC.T..7th.ed..2010.2015.) & Derived.AJCC.T..7th.ed..2010.2015. == "TX") |
                   (!is.na(Derived.AJCC.T..6th.ed..2004.2015.) & Derived.AJCC.T..6th.ed..2004.2015. == "TX")
               ) &
                 !(
                   (!is.na(Derived.AJCC.N..7th.ed..2010.2015.) & Derived.AJCC.N..7th.ed..2010.2015. == "NX") |
                     (!is.na(Derived.AJCC.N..6th.ed..2004.2015.) & Derived.AJCC.N..6th.ed..2004.2015. == "NX")
                 )
)
...
# Add 6th edition variables to the list
vars_to_summarize <- c(
...
summarize_variable <- function(varname) {
  ...
  return(df[, c("Variable", "Category", "Summary")])
}
# Run summary again
...
cat("✔️ Summary table with 6th edition variables saved as 'variable_summary_table.csv'.\n")




# Combine T, N, M Stages and Recode for Table 1

# 1. Combine T stage using 7th then 6th edition
combine_T_stage <- function(row) {
  if (!is.na(row["Derived.AJCC.T..7th.ed..2010.2015."]) && row["Derived.AJCC.T..7th.ed..2010.2015."] != "TX") {
    return(row["Derived.AJCC.T..7th.ed..2010.2015."])
  } else if (!is.na(row["Derived.AJCC.T..6th.ed..2004.2015."]) && row["Derived.AJCC.T..6th.ed..2004.2015."] != "TX") {
...
combine_N_stage <- function(row) {
  if (!is.na(row["Derived.AJCC.N..7th.ed..2010.2015."]) && row["Derived.AJCC.N..7th.ed..2010.2015."] != "NX") {
    return(row["Derived.AJCC.N..7th.ed..2010.2015."])
  } else if (!is.na(row["Derived.AJCC.N..6th.ed..2004.2015."]) && row["Derived.AJCC.N..6th.ed..2004.2015."] != "NX") {
...
# Recode chemotherapy
data$Chemotherapy <- ifelse(data$Chemotherapy.recode..yes..no.unk. == "Yes", "Yes", "No/unknown")

# Recode radiation into binary: Yes if received radiation, No otherwise
data$Radiation <- dplyr::case_when(
  data$Radiation.recode %in% c("Beam radiation",
  ...
  TRUE ~ "No"  # Includes None/Unknown, Recommended but unknown, Refused
)


# Recode metastasis variables
...
# Recode race
data$Race_grouped <- ifelse(data$Race.recode..White..Black..Other. == "White", "White",
                            ifelse(data$Race.recode..White..Black..Other. == "Black", "Black", "Other"))

# Recode marital status
data$Marital_grouped <- ifelse(data$Marital.status.at.diagnosis == "Married (including common law)", "Married", "Others")

# Recode Age
data$Age_grouped <- case_when(
  data$Age.recode.with..1.year.olds %in% c("20-24 years", "25-29 years", "30-34 years",
                                           "35-39 years", "40-44 years", "45-49 years",
                                           "50-54 years") ~ "20–54 years",
...
# Recode histology
data$Histology_grouped <- ifelse(data$Histology_Group == "Adenocarcinoma", "Adenocarcinoma",
                                 ifelse(data$Histology_Group == "Squamous Cell Carcinoma", "Squamous", "Others"))

# Recode grade
data$Grade_grouped <- ifelse(data$Grade.Recode..thru.2017. == "Well differentiated; Grade I", "Grade I",
                             ifelse(data$Grade.Recode..thru.2017. == "Moderately differentiated; Grade II", "Grade II",
                                    ifelse(data$Grade.Recode..thru.2017. == "Poorly differentiated; Grade III", "Grade III",
                                           ifelse(data$Grade.Recode..thru.2017. == "Undifferentiated; anaplastic; Grade IV", "Grade IV", NA))))


# Recode primary site
data$Primary_site_grouped <- ifelse(data$Primary.Site...labeled == "C34.0-Main bronchus", "Main bronchus",
                                    ifelse(data$Primary.Site...labeled == "C34.1-Upper lobe, lung", "Upper lobe",
                                           ifelse(data$Primary.Site...labeled == "C34.2-Middle lobe, lung", "Middle lobe",
                                                  ifelse(data$Primary.Site...labeled == "C34.3-Lower lobe, lung", "Lower lobe",
                                                         ifelse(data$Primary.Site...labeled == "C34.8-Overlapping lesion of lung", "Overlapping lesion of lung", NA)))))


# Recode Urban/Rural ###
data$Urban_Rural <- ifelse(data$Rural.Urban.Continuum.Code %in% c(
  ...
  "Urban",
  ifelse(data$Rural.Urban.Continuum.Code %in% c(
...
data$Income_group <- case_when(
  data$Median.household.income.inflation.adj.to.2022 %in% c(
    "< $40,000", "$40,000 - $44,999", "$45,000 - $49,999", "$50,000 - $54,999") ~ "< $55,000",
  
  data$Median.household.income.inflation.adj.to.2022 %in% c(
    "$55,000 - $59,999", "$60,000 - $64,999", "$65,000 - $69,999", "$70,000 - $74,999") ~ "$55,000 - $75,000",
  
  data$Median.household.income.inflation.adj.to.2022 %in% c(
    "$75,000 - $79,999", "$80,000 - $84,999", "$85,000 - $89,999") ~ "$75,000 - $89,999",
  
  data$Median.household.income.inflation.adj.to.2022 %in% c(
    "$90,000 - $94,999", "$95,000 - $99,999", "$100,000 - $109,999", "$110,000 - $119,999", "$120,000+") ~ "> $90,000",
